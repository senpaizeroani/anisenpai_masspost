...
if ("createEvent" in document) {
    ...
    countEpObj.dispatchEvent(evt);
}
else
    countEpObj.fireEvent("onchange");
//countEpObj.onchange();
...
<div class="box1">
  ...
  <div>
    <p>Links</p>
  	<textarea id="MasslinkerTextRaw" placeholder="Jede Linie entspricht einer Episode. Mehrere Hoster mit , trenne."
    onkeyup="getLinks()"
    onchange="getLinks()"
  ...
  <div>
    <button onclick="selectKategory(document.getElementById("allKategories").value);submitMassPost()">SenpaiZeros Masslinker Starten</button>
  </div>
</div>
`;
var wpIsHidden = true;
...
let getNameFromLink = link => {
  if(link.includes("vivo")){
    ...
    return "Mixdrop";
  }else if(link.includes("ani-play")){
    return "AniPlay";
...
let getLinks = () => {
  ...
  if(links.length > 300){
  	alert("Max 300 Post's are allowed");
  	MasslinkerTextRawObj.value = "";
  	getLinks();
  	return;
  }
